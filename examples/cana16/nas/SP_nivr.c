...
#pragma scop
if ((((N1 >= 1) && (N2 >= 1)) && (N3 >= 1))) {
  for (c1 = 0; c1 <= floord(N1, 16); c1++) {
    for (c2 = 0; c2 <= floord(N2, 16); c2++) {
      for (c3 = 0; c3 <= floord(N3, 16); c3++) {
        for (c4 = max(1, (16 * c1)); c4 <= min(N1, ((16 * c1) + 15)); c4++) {
          for (c5 = max(1, (16 * c2)); c5 <= min(N2, ((16 * c2) + 15)); c5++) {
            for (c6 = max(1, (16 * c3)); c6 <= min(N3, ((16 * c3) + 15)); c6++) {
              r4 = rhs[4][c6][c5][c4];
              r5 = rhs[5][c6][c5][c4];
              r3 = rhs[3][c6][c5][c4];
              t2 = 0.5 * ( r4 + r5 );
