...
for (c0 = 0; c0 <= (M - 1)/ 32; c0 += 1)
  for (c1 = 0; c1 <= (M - 1) / 32; c1 += 1)
    for (c2 = 32 * c0; c2 <= min(M - 1, 32 * c0 + 31); c2 += 1)
      for (c3 = 32 * c1; c3 <= min(32 * c1 + 31, M - 1); c3 += 1) {
        temp2d[c2][c3]=0.0;
        sum2[c2][c3]=0.0;
      }
if (M >= 33)
#pragma omp parallel for
  for (c2 = 0; c2 <= (M - 1) / 32; c2 += 1)
    if (M >= 33 && c2 >= 0)
      for (c0 = 0; c0 <= (M - 1) / 32; c0 += 1)
        for (c4 = 0; c4 <= (M - 1) / 32; c4 += 1)
          for (c6 = 32 * c0; c6 <= min(M - 1, 32 * c0 + 31); c6 += 1)
            for (c8 = 32 * c2; c8 <= min(32 * c2 + 31, M - 1); c8 += 1)
              for (c10 = 32 * c4; c10 <= min(M - 1, 32 * c4 + 31); c10 += 1) {
                if (c10 == 0 && c4 == 0 && c6 >= 32) {
