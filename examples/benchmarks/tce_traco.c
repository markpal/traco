...
for (c0 = 0; c0 <= (N - 1) / 32; c0 += 1)
  for (c2 = 0; c2 <= (N - 1) / 32; c2 += 1)
    for (c4 = 0; c4 <= (N - 1) / 32; c4 += 1)
      for (c6 = 0; c6 <= (N - 1) / 32; c6 += 1)
        for (c8 = 0; c8 <= (N - 1) / 32; c8 += 1)
          for (c10 = 32 * c0; c10 <= min(N - 1, 32 * c0 + 31); c10 += 1)
            for (c12 = 32 * c2; c12 <= min(32 * c2 + 31, N - 1); c12 += 1)
              for (c14 = 32 * c4; c14 <= min(N - 1, 32 * c4 + 31); c14 += 1)
                for (c16 = 32 * c8; c16 <= min(N - 1, 32 * c8 + 31); c16 += 1)
                  for (c18 = 32 * c6; c18 <= min(32 * c6 + 31, N - 1); c18 += 1) {
