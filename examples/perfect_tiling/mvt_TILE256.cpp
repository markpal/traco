...
for (int c0 = 0; c0 <= floord(NX - 1, 256); c0 += 1)
  for (int c1 = 0; c1 <= (NX - 1) / 256; c1 += 1)
    for (int c2 = 256 * c0; c2 <= min(NX - 1, 256 * c0 + 255); c2 += 1)
      for (int c3 = 256 * c1; c3 <= min(256 * c1 + 255, NX - 1); c3 += 1) {
...
/* Start of CLooG code */
if (NX >= 1) {
  for (t1=0;t1<=floord(NX-1,256);t1++) {
    for (t2=0;t2<=floord(NX-1,256);t2++) {
      for (t3=256*t1;t3<=min(NX-1,256*t1+255);t3++) {
        for (t4=256*t2;t4<=min(NX-1,256*t2+255);t4++) {
