...
for (c0 = 0; c0 <= floord(N1 - 1, 64); c0 += 1)
  for (c1 = 0; c1 <= floord(N2 - 1, 64); c1 += 1)
    for (c2 = 0; c2 <= floord(N3 - 1, 64); c2 += 1)
        for (c4 = 64 * c1 + 1; c4 <= min(N2, 64 * c1 + 64); c4 += 1)
           for (c5 = 64 * c2 + 1; c5 <= min(N3, 64 * c2 + 64); c5 += 1)
             for (c3 = 64 * c0 + 1; c3 <= min(N1, 64 * c0 + 64); c3 += 1)
          {
...
/* Start of CLooG code */
if ((N1 >= 1) && (N2 >= 1) && (N3 >= 1)) {
  for (t1=0;t1<=floord(N1,64);t1++) {
    for (t2=0;t2<=floord(N2,64);t2++) {
      for (t3=0;t3<=floord(N3,64);t3++) {
        for (t4=max(1,64*t2);t4<=min(N2,64*t2+63);t4++) {
          for (t5=max(1,64*t3);t5<=min(N3,64*t3+63);t5++) {
            lbv=max(1,64*t1);
            ubv=min(N1,64*t1+63);
#pragma ivdep
