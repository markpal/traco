...
/* Start of CLooG code */
  ...
  for (int t2=0;t2<=floord(NI-1,16);t2++) {
    for (int t3=0;t3<=floord(NI-1,16);t3++) {
      for (int t4=16*t2;t4<=min(NI-1,16*t2+15);t4++) {
        for (int t10=16*t3;t10<=min(NI-1,16*t3+15);t10++) {
          C[t4][t10]*=beta;
        }
      }
    }
    ...
    for (int t2=0;t2<=floord(NI-1,16);t2++) {
      for (int t3=0;t3<=floord(NI-1,16);t3++) {
        for (int t4=0;t4<=floord(NJ-1,16);t4++) {
          for (int t5=16*t2;t5<=min(16*t2+15,NI-1);t5++) {
            for (int t6=16*t3;t6<=min(16*t3+15,NI-1);t6++) {
              for (int t7=16*t4;t7<=min(16*t4+15,NJ-1);t7++) {   //very amazing, changing order of arguments in 'min' slows the loop strongly
