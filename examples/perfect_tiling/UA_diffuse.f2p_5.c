      ...
      for (c3 = 0; c3 <= floord(N4 - 1, 16); c3 += 1)
        for (c4 = 16 * c0 + 1; c4 <= min(N1, 16 * c0 + 16); c4 += 1)
          for (c5 = 16 * c2 + 1; c5 <= min(N3, 16 * c2 + 16); c5 += 1)
            for (c6 = 16 * c3 + 1; c6 <= min(N4, 16 * c3 + 16); c6 += 1)
              for (c7 = 16 * c1 + 1; c7 <= min(N2, 16 * c1 + 16); c7 += 1)
...
if ((N1 >= 1) && (N2 >= 1) && (N3 >= 1) && (N4 >= 1)) {
  for (t1=0;t1<=floord(N2,16);t1++) {
    for (t2=0;t2<=floord(N3,16);t2++) {
      for (t3=0;t3<=floord(N4,16);t3++) {
        for (t4=0;t4<=floord(N1,16);t4++) {
          for (t5=max(1,16*t2);t5<=min(N3,16*t2+15);t5++) {
            for (t6=max(1,16*t3);t6<=min(N4,16*t3+15);t6++) {
              for (t7=max(1,16*t4);t7<=min(N1,16*t4+15);t7++) {
                lbv=max(1,16*t1);
                ubv=min(N2,16*t1+15);
#pragma ivdep
