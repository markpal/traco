...
for (c0 = 0; c0 <= floord(NX - 1, 16); c0 += 1)
  for (c1 = 0; c1 <= (NX - 1) / 16; c1 += 1)
    for (c2 = 16 * c0; c2 <= min(NX - 1, 16 * c0 + 15); c2 += 1)
      for (c3 = 16 * c1; c3 <= min(16 * c1 + 15, NX - 1); c3 += 1)
        A[c2][c3]=A[c2][c3]+u1[c2]*v1[c3]+u2[c2]*v2[c3];


for (c0 = 0; c0 <= floord(NX - 1, 16); c0 += 1)
  for (c1 = 0; c1 <= (NX - 1) / 16; c1 += 1)
    for (c2 = 16 * c1; c2 <= min(NX - 1, 16 * c1 + 15); c2 += 1)
      for (c3 = 16 * c0; c3 <= min(16 * c0 + 15, NX - 1); c3 += 1)
...
if (NX >= 1) {
  for (t2=0;t2<=floord(NX-1,16);t2++) {
    for (t3=0;t3<=floord(NX-1,16);t3++) {
      for (t4=16*t3;t4<=min(NX-1,16*t3+15);t4++) {
        lbv=16*t2;
        ubv=min(NX-1,16*t2+15);
#pragma ivdep
  ...
  for (t2=0;t2<=floord(NX-1,16);t2++) {
    for (t3=0;t3<=floord(NX-1,16);t3++) {
      for (t4=16*t2;t4<=min(NX-1,16*t2+15);t4++) {
        for (t5=16*t3;t5<=min(NX-1,16*t3+15);t5++) {
