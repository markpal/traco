...
/* Start of CLooG code */
if ((N1 >= 1) && (N2 >= 1) && (N3 >= 1)) {
  for (t1=0;t1<=floord(N1,4);t1++) {
    for (t2=0;t2<=floord(N2,4);t2++) {
      for (t3=0;t3<=floord(N3,4);t3++) {
        for (t4=max(1,4*t1);t4<=min(N1,4*t1+3);t4++) {
...
for (c0 = 0; c0 <= floord(N1 - 1, 4); c0 += 1)
  for (c1 = 0; c1 <= floord(N2 - 1, 4); c1 += 1)
    for (c2 = 0; c2 <= floord(N3 - 1, 4); c2 += 1)
      for (c3 = 4 * c0 + 1; c3 <= min(N1, 4 * c0 + 4); c3 += 1)
        for (c4 = 4 * c2 + 1; c4 <= min(N3, 4 * c2 + 4); c4 += 1)
          for (c5 = 4 * c1 + 1; c5 <= min(N2, 4 * c1 + 4); c5 += 1)
            wdtdr[c3][c5]=wdtdr[c3][c5]+wxm1[c4]*dxm1[c4][c3]*dxm1[c4][c5];
