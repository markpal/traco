...
for (c0 = 0; c0 <= ub; c0 += 1)
  if (N >= 200 * c0 + 1 && c0 >= 0)
    for (c0_0 = 0; c0_0 <= (N - 1) / 200; c0_0 += 1) {
      if (c0 >= 1 && c0_0 >= c0 + 1) {
        for (c4 = 200 * c0_0; c4 <= min(200 * c0_0 + 199, N - 1); c4 += 1)
          for (c6 = 0; c6 <= 15; c6 += 1)
            x[c6]=x[c6]+beta*B[c6][c4]*y[c4];
      } else if (c0_0 == c0)
        for (c2 = 0; c2 < 25 * c0; c2 += 1)
          for (c4 = 200 * c0; c4 <= min(200 * c0 + 199, N - 1); c4 += 1)
      ...
      if (c0_0 == c0)
        for (c2 = 25 * c0 + 25; c2 <= (N - 1) / 16; c2 += 1)
          for (c4 = 200 * c0; c4 <= 200 * c0 + 199; c4 += 1)
            for (c6 = 16 * c2; c6 <= min(N - 1, 16 * c2 + 15); c6 += 1)
              B[c4][c6]=A[c4][c6]+u1[c4]*v1[c6]+u2[c4]*v2[c6];
    }
...
for (c0 = 0; c0 <= ub; c0 += 1)
  for (c1 = 0; c1 <= (N - 1) / 16; c1 += 1)
    for (c2 = 16 * c1; c2 <= min(N - 1, 16 * c1 + 15); c2 += 1)
      for (c3 = 200 * c0; c3 <= min(200 * c0 + 199, N - 1); c3 += 1)
