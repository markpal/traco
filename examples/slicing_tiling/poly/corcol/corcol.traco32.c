...
int main(int argc, char** argv)
{
 ...
 int ub = floord(M - 2, 32);
#pragma omp parallel for
for (c0 = 0; c0 <= ub; c0 += 1) {
  for (c6 = 32 * c0 + 1; c6 <= min(M - 1, 32 * c0 + 32); c6 += 1)
    symmat[c6][c6]=1.0;
  for (c2 = 0; c2 <= -c0 + (M - 2) / 32; c2 += 1)
    for (c6 = 32 * c0 + 1; c6 <= min(32 * c0 + 32, M - 32 * c2 - 1); c6 += 1)
      for (c10 = 32 * c2 + c6 + 1; c10 <= min(M, 32 * c2 + c6 + 32); c10 += 1)
        symmat[c6][c10]=0.0;
  for (c2 = 0; c2 <= -c0 + (M - 2) / 32; c2 += 1)
