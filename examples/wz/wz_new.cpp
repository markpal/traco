...
for( c1 = 0; c1 < b; c1 += 1) {
  ...
  #pragma omp parallel for private(det, c3,c4,c5,c9,c11) shared(c1,a,w)
  for( c3 = 0; c3 <= (n - 2 * c1 - 3)/ 16; c3 += 1)
    for( c4 = 0; c4 <= 1; c4 += 1) {
      if (c4 == 1) {
        for( c5 = 0; c5 <= (n - 2 * c1 - 3) / 16; c5 += 1)
          for( c9 = c1 + 16 * c3 + 2; c9 <= min(n - c1 - 1, c1 + 16 * c3 + 17); c9 += 1)
            for( c11 = c1 + 16 * c5 + 2; c11 <= min(n - c1 - 1, c1 + 16 * c5 + 17); c11 += 1)
