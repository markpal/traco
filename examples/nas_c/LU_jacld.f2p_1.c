...
for(j = N1; j <= N2; j++){
  for(i = N3; i <= N4; i++){
    ...
    tmp3 = tmp1 * tmp2;
    ;
    d[1][1][i][j] =  1.0 + dt * 2.0 * (   tx1 * dx1 + ty1 * dy1 + tz1 * dz1 );
    d[1][2][i][j] =  0.0;
    d[1][3][i][j] =  0.0;
    d[1][4][i][j] =  0.0;
    d[1][5][i][j] =  0.0;
    ;
    d[2][1][i][j] = -dt * 2.0 * (  tx1 * r43 + ty1 + tz1  ) * c34 * tmp2 * u[2][i][j][k];
    d[2][2][i][j] =  1.0 + dt * 2.0 * c34 * tmp1 * (  tx1 * r43 + ty1 + tz1 ) + dt * 2.0 * (   tx1 * dx2 + ty1 * dy2 + tz1 * dz2  );
    d[2][3][i][j] = 0.0;
    d[2][4][i][j] = 0.0;
    d[2][5][i][j] = 0.0;
    ;
    d[3][1][i][j] = -dt * 2.0 * (  tx1 + ty1 * r43 + tz1  ) * c34 * tmp2 * u[3][i][j][k];
    d[3][2][i][j] = 0.0;
    d[3][3][i][j] = 1.0 + dt * 2.0 * c34 * tmp1 * (  tx1 + ty1 * r43 + tz1 ) + dt * 2.0 * (  tx1 * dx3 + ty1 * dy3 + tz1 * dz3 );
    d[3][4][i][j] = 0.0;
    d[3][5][i][j] = 0.0;
    ;
    d[4][1][i][j] = -dt * 2.0 * (  tx1 + ty1 + tz1 * r43  ) * c34 * tmp2 * u[4][i][j][k];
    d[4][2][i][j] = 0.0;
    d[4][3][i][j] = 0.0;
    d[4][4][i][j] = 1.0 + dt * 2.0 * c34 * tmp1 * (  tx1 + ty1 + tz1 * r43 ) + dt * 2.0 * (  tx1 * dx4 + ty1 * dy4 + tz1 * dz4 );
    d[4][5][i][j] = 0.0;
    ;
    d[5][1][i][j] = -dt * 2.0 * ( ( ( tx1 * ( r43*c34 - c1345 ) + ty1 * ( c34 - c1345 ) + tz1 * ( c34 - c1345 ) ) * ( u[2][i][j][k] ** 2 ) + ( tx1 * ( c34 - c1345 ) + ty1 * ( r43*c34 - c1345 ) + tz1 * ( c34 - c1345 ) ) * ( u[3][i][j][k] ** 2 ) + ( tx1 * ( c34 - c1345 ) + ty1 * ( c34 - c1345 ) + tz1 * ( r43*c34 - c1345 ) ) * ( u[4][i][j][k] ** 2 ) ) * tmp3 + ( tx1 + ty1 + tz1 ) * c1345 * tmp2 * u[5][i][j][k] );
    ;
    d[5][2][i][j] = dt * 2.0 * tmp2 * u[2][i][j][k] * ( tx1 * ( r43*c34 - c1345 ) + ty1 * (     c34 - c1345 ) + tz1 * (     c34 - c1345 ) );
    d[5][3][i][j] = dt * 2.0 * tmp2 * u[3][i][j][k] * ( tx1 * ( c34 - c1345 ) + ty1 * ( r43*c34 -c1345 ) + tz1 * ( c34 - c1345 ) );
    d[5][4][i][j] = dt * 2.0 * tmp2 * u[4][i][j][k] * ( tx1 * ( c34 - c1345 ) + ty1 * ( c34 - c1345 ) + tz1 * ( r43*c34 - c1345 ) );
    d[5][5][i][j] = 1.0 + dt * 2.0 * ( tx1  + ty1 + tz1 ) * c1345 * tmp1 + dt * 2.0 * (  tx1 * dx5 +  ty1 * dy5 +  tz1 * dz5 );
    ;
    tmp1 = rho_i[i][j][k-1];
    tmp2 = tmp1 * tmp1;
    tmp3 = tmp1 * tmp2;
    ...
    a[4][4][i][j] = - dt * tz2 * ( 2.0 - c2 ) * ( u[4][i][j][k-1] * tmp1 ) - dt * tz1 * ( r43 * c34 * tmp1 ) - dt * tz1 * dz4;
    a[4][5][i][j] = - dt * tz2 * c2;
    ;
    a[5][1][i][j] = - dt * tz2 * ( ( c2 * 2.0 * qs[i][j][k-1] - c1 * u[5][i][j][k-1] ) * u[4][i][j][k-1] * tmp2 ) - dt * tz1 * ( - ( c34 - c1345 ) * tmp3 * (u[2][i][j][k-1]**2) - ( c34 - c1345 ) * tmp3 * (u[3][i][j][k-1]**2) - ( r43*c34 - c1345 )* tmp3 * (u[4][i][j][k-1]**2) - c1345 * tmp2 * u[5][i][j][k-1] );
    ...
    b[3][3][i][j] = - dt * ty2 * ( ( 2.0 - c2 ) * ( u[3][i][j-1][k] * tmp1 ) ) - dt * ty1 * ( r43 * c34 * tmp1 ) - dt * ty1 * dy3;
    ...
    b[4][5][i][j] = 0.0;
    ;
    b[5][1][i][j] = - dt * ty2 * ( ( c2 * 2.0 * qs[i][j-1][k] - c1 * u[5][i][j-1][k] ) * ( u[3][i][j-1][k] * tmp2 ) ) - dt * ty1 * ( - (     c34 - c1345 )*tmp3*(u[2][i][j-1][k]**2) - ( r43*c34 - c1345 )*tmp3*(u[3][i][j-1][k]**2) - (     c34 - c1345 )*tmp3*(u[4][i][j-1][k]**2) - c1345*tmp2*u[5][i][j-1][k] );
    ...
    c[2][2][i][j] = - dt * tx2 * ( ( 2.0 - c2 ) * ( u[2][i-1][j][k] * tmp1 ) ) - dt * tx1 * ( r43 * c34 * tmp1 ) - dt * tx1 * dx2;
    ...
    c[4][5][i][j] = 0.0;
    ;
    c[5][1][i][j] = - dt * tx2 * ( ( c2 * 2.0 * qs[i-1][j][k] - c1 * u[5][i-1][j][k] ) * u[2][i-1][j][k] * tmp2 ) - dt * tx1 * ( - ( r43*c34 - c1345 ) * tmp3 * ( u[2][i-1][j][k]**2 ) - (     c34 - c1345 ) * tmp3 * ( u[3][i-1][j][k]**2 ) - (     c34 - c1345 ) * tmp3 * ( u[4][i-1][j][k]**2 ) - c1345 * tmp2 * u[5][i-1][j][k] );
