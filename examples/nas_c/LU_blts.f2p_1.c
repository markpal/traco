...
for(j = N1; j <= N2; j++){
  for(i = N3; i <= N4; i++){
    for(m = 1; m <= 5; m++){
      ;
      tv[ m ] =  v[ m][ i][ j][ k ] - omega * (  ldz[ m][ 1][ i][ j ] * v[ 1][ i][ j][ k-1 ] + ldz[ m][ 2][ i][ j ] * v[ 2][ i][ j][ k-1 ] + ldz[ m][ 3][ i][ j ] * v[ 3][ i][ j][ k-1 ] + ldz[ m][ 4][ i][ j ] * v[ 4][ i][ j][ k-1 ] + ldz[ m][ 5][ i][ j ] * v[ 5][ i][ j][ k-1 ]  );
      ;
    }
    ;
    ;
    for(m = 1; m <= 5; m++){
      ;
      tv[ m ] =  tv[ m ] - omega * ( ldy[ m][ 1][ i][ j ] * v[ 1][ i][ j-1][ k ] + ldx[ m][ 1][ i][ j ] * v[ 1][ i-1][ j][ k ] + ldy[ m][ 2][ i][ j ] * v[ 2][ i][ j-1][ k ] + ldx[ m][ 2][ i][ j ] * v[ 2][ i-1][ j][ k ] + ldy[ m][ 3][ i][ j ] * v[ 3][ i][ j-1][ k ] + ldx[ m][ 3][ i][ j ] * v[ 3][ i-1][ j][ k ] + ldy[ m][ 4][ i][ j ] * v[ 4][ i][ j-1][ k ] + ldx[ m][ 4][ i][ j ] * v[ 4][ i-1][ j][ k ] + ldy[ m][ 5][ i][ j ] * v[ 5][ i][ j-1][ k ] + ldx[ m][ 5][ i][ j ] * v[ 5][ i-1][ j][ k ] );
      ;
    }
    ;
    for(m = 1; m <= 5; m++){
