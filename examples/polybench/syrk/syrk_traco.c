...
for (c0 = 0; c0 <= floord(NI - 1, 32); c0 += 1) {
  for (c2 = 0; c2 <= (NI - 1) / 32; c2 += 1)
    for (c6 = 32 * c0; c6 <= min(NI - 1, 32 * c0 + 31); c6 += 1)
      for (c8 = 32 * c2; c8 <= min(32 * c2 + 31, NI - 1); c8 += 1)
        c[c6][c8]*=beta;
  for (c2 = 0; c2 <= (NI - 1) / 32; c2 += 1)
    for (c4 = 0; c4 <= floord(NJ - 1, 32); c4 += 1)
      for (c6 = 32 * c0; c6 <= min(NI - 1, 32 * c0 + 31); c6 += 1)
        for (c8 = 32 * c4; c8 <= min(NJ - 1, 32 * c4 + 31); c8 += 1)
          for (c10 = 32 * c2; c10 <= min(32 * c2 + 31, NI - 1); c10 += 1) {
