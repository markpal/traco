...
int i,j,k, alpha=0, beta=0;
 ...
 #pragma acc data copyinout(C[0:N*N], acc[0:N*N]) copyin(alpha, A[0:N*N], B[0:N*N], N, NI, NJ, alpha,beta) private(i,j,k)
 {
 if(NI > 3 && NJ > 3)
   for (i = 0; i < NI; i++)
    for (j = 0; j < NJ; j++)
...
*/
 #pragma acc data copyinout(C[0:N*N], acc[0:N*N]) copyin(alpha, A[0:N*N], B[0:N*N], N, NI, NJ, alpha,beta)
 {
if (NJ >= 4 && NI >= 4)
