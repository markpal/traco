...
void comp_pluto(float A[DIM1][DIM2][DIM3], float sum[DIM1][DIM2][DIM3], float C4[DIM1][DIM2]) {
int t1, t2, t3, t4, t5;
...
#pragma omp parallel for private(lbv,ubv)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t3=0;t3<=N-1;t3++) {
      for (t4=0;t4<=N-1;t4++) {
        for (t5=0;t5<=N-1;t5++) {
          sum[t2][t3][t4]=sum[t2][t3][t4]+A[t2][t3][t5]*C4[t5][t4];;
        }
      }
    }
  }
