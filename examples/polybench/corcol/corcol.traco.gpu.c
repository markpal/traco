...
int main(int argc, char** argv)
{
  ...
  #pragma acc data copyinout(symmat[0:n*n]) copyin(M,N,n,data[0:n*n])
  {
  #pragma acc kernels loop private(j1)
  for (j1 = 1; j1 <= M-1; j1++)
     symmat[j1][j1] = 1.0;

  #pragma acc kernels loop private(j1,j2)
  for (j1 = 1; j1 <= M-1; j1++)
  #pragma acc loop
    for (j2 = j1+1; j2 <= M; j2++)
     symmat[j1][j2] = 0.0;

  #pragma acc kernels loop private(j1,j2,i)
  for (j1 = 1; j1 <= M-1; j1++)
   #pragma acc loop
      for (i = 1; i <= N; i++)
       #pragma acc loop
