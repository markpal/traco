...
/* Start of CLooG code */
if (M >= 2) {
  for (t2=0;t2<=floord(M-1,4);t2++) {
    for (t3=t2;t3<=floord(M,4);t3++) {
      for (t4=max(1,4*t2);t4<=min(min(M-1,4*t2+3),4*t3+2);t4++) {
        lbv=max(4*t3,t4+1);
        ubv=min(M,4*t3+3);
#pragma ivdep
#pragma vector always
  ...
  if (N >= 1) {
    for (t2=0;t2<=floord(M-1,4);t2++) {
      for (t3=t2;t3<=floord(M,4);t3++) {
        for (t4=0;t4<=floord(N,4);t4++) {
          for (t5=max(1,4*t2);t5<=min(min(M-1,4*t2+3),4*t3+2);t5++) {
            for (t6=max(1,4*t4);t6<=min(N,4*t4+3);t6++) {
              lbv=max(4*t3,t5+1);
              ubv=min(M,4*t3+3);
#pragma ivdep
#pragma vector always
