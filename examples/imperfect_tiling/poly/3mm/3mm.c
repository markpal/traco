...
for (c0 = 0; c0 <= floord(NI - 1, 16); c0 += 1)
  for (c1 = 0; c1 <= floord(NJ - 1, 16); c1 += 1) {
    for (c2 = 0; c2 <= floord(NK - 1, 16); c2 += 1)
      for (c3 = 16 * c0; c3 <= min(NI - 1, 16 * c0 + 15); c3 += 1) {
        for (c4 = 16 * c2; c4 <= min(NK - 1, 16 * c2 + 15); c4 += 1) {
          if (c2 == 0 && c1 == 0 && NJ >= c4 + 1)
            E[c3][c4]=0;
          for (c5 = 16 * c1; c5 <= min(NJ - 1, 16 * c1 + 15); c5 += 1)
            E[c3][c5]=E[c3][c5]+A[c3][c4]*B[c4][c5];
        }
        if (c2 == 0) {
          for (c4 = max(16, 16 * c1); c4 <= min(min(NJ - 1, NK - 1), 16 * c1 + 15); c4 += 1)
            E[c3][c4]=0;
          for (c4 = max(16 * c1, NK); c4 <= min(NJ - 1, 16 * c1 + 15); c4 += 1)
            E[c3][c4]=0;
        }
      }
  }
for (c0 = 0; c0 <= floord(NJ - 1, 16); c0 += 1)
  for (c1 = 0; c1 <= floord(NL - 1, 16); c1 += 1) {
    for (c2 = 0; c2 <= floord(NM - 1, 16); c2 += 1)
      for (c3 = 16 * c0; c3 <= min(NJ - 1, 16 * c0 + 15); c3 += 1) {
        for (c4 = 16 * c2; c4 <= min(NM - 1, 16 * c2 + 15); c4 += 1) {
          if (c2 == 0 && c1 == 0 && NL >= c4 + 1)
            F[c3][c4]=0;
          for (c5 = 16 * c1; c5 <= min(NL - 1, 16 * c1 + 15); c5 += 1)
            F[c3][c5]=F[c3][c5]+C[c3][c4]*D[c4][c5];
        }
        if (c2 == 0) {
          for (c4 = max(16, 16 * c1); c4 <= min(min(NL - 1, NM - 1), 16 * c1 + 15); c4 += 1)
            F[c3][c4]=0;
          for (c4 = max(16 * c1, NM); c4 <= min(NL - 1, 16 * c1 + 15); c4 += 1)
            F[c3][c4]=0;
        }
      }
    if (NM <= 0)
      for (c3 = 16 * c0; c3 <= min(NJ - 1, 16 * c0 + 15); c3 += 1)
        for (c4 = 16 * c1; c4 <= min(NL - 1, 16 * c1 + 15); c4 += 1)
          F[c3][c4]=0;
  }
  ...
  for (c1 = 0; c1 <= floord(NL - 1, 16); c1 += 1) {
    for (c2 = 0; c2 <= floord(NJ - 1, 16); c2 += 1)
      for (c3 = 16 * c0; c3 <= min(NI - 1, 16 * c0 + 15); c3 += 1) {
        ...
        if (c2 == 0) {
          for (c4 = max(16, 16 * c1); c4 <= min(min(NL - 1, NJ - 1), 16 * c1 + 15); c4 += 1)
            G[c3][c4]=0;
          for (c4 = max(16 * c1, NJ); c4 <= min(NL - 1, 16 * c1 + 15); c4 += 1)
            G[c3][c4]=0;
        }
      }
    if (NJ <= 0)
...
/* Start of CLooG code */
if ((NI >= 0) && (NJ >= 0) && (NL >= 1)) {
  for (t2=0;t2<=floord(NI+NJ-1,16);t2++) {
    for (t3=0;t3<=floord(NL-1,16);t3++) {
      for (t4=16*t2;t4<=min(min(NI-1,NJ-1),16*t2+15);t4++) {
        lbv=16*t3;
        ubv=min(NL-1,16*t3+15);
#pragma ivdep
#pragma vector always
      ...
      for (t4=max(NI,16*t2);t4<=min(NJ-1,16*t2+15);t4++) {
        lbv=16*t3;
        ubv=min(NL-1,16*t3+15);
#pragma ivdep
#pragma vector always
      ...
      for (t4=max(NJ,16*t2);t4<=min(NI-1,16*t2+15);t4++) {
        lbv=16*t3;
        ubv=min(NL-1,16*t3+15);
#pragma ivdep
#pragma vector always
...
if ((NI <= -1) && (NL >= 1)) {
  for (t2=0;t2<=floord(NJ-1,16);t2++) {
    for (t3=0;t3<=floord(NL-1,16);t3++) {
      for (t4=16*t2;t4<=min(NJ-1,16*t2+15);t4++) {
        lbv=16*t3;
        ubv=min(NL-1,16*t3+15);
#pragma ivdep
#pragma vector always
...
if ((NJ <= -1) && (NL >= 1)) {
  for (t2=0;t2<=floord(NI-1,16);t2++) {
    for (t3=0;t3<=floord(NL-1,16);t3++) {
      for (t4=16*t2;t4<=min(NI-1,16*t2+15);t4++) {
        lbv=16*t3;
        ubv=min(NL-1,16*t3+15);
#pragma ivdep
#pragma vector always
...
if ((NL >= 1) && (NM >= 1)) {
  for (t2=0;t2<=floord(NJ-1,16);t2++) {
    for (t3=0;t3<=floord(NL-1,16);t3++) {
      for (t5=0;t5<=floord(NM-1,16);t5++) {
        for (t6=16*t2;t6<=min(NJ-1,16*t2+15);t6++) {
          for (t8=16*t5;t8<=min(NM-1,16*t5+15);t8++) {
            lbv=16*t3;
            ubv=min(NL-1,16*t3+15);
#pragma ivdep
#pragma vector always
...
if (NJ >= 1) {
  for (t2=0;t2<=floord(NI-1,16);t2++) {
    for (t3=0;t3<=floord(NJ-1,16);t3++) {
      for (t4=16*t2;t4<=min(NI-1,16*t2+15);t4++) {
        lbv=16*t3;
        ubv=min(NJ-1,16*t3+15);
#pragma ivdep
#pragma vector always
        for (t17=lbv;t17<=ubv;t17++) {
          E[t4][t17]=0;;
...
if (NJ >= 1) {
  for (t2=0;t2<=floord(NI-1,16);t2++) {
    for (t3=0;t3<=floord(NJ-1,16);t3++) {
      if (NK >= 1) {
        for (t5=0;t5<=floord(NK-1,16);t5++) {
          for (t6=16*t2;t6<=min(NI-1,16*t2+15);t6++) {
            for (t7=16*t3;t7<=min(NJ-1,16*t3+15);t7++) {
              for (t9=16*t5;t9<=min(NK-1,16*t5+15);t9++) {
      ...
      if (NL >= 1) {
        for (t5=0;t5<=floord(NL-1,16);t5++) {
          for (t6=16*t2;t6<=min(NI-1,16*t2+15);t6++) {
            for (t7=16*t3;t7<=min(NJ-1,16*t3+15);t7++) {
              lbv=16*t5;
              ubv=min(NL-1,16*t5+15);
#pragma ivdep
#pragma vector always
              for (t17=lbv;t17<=ubv;t17++) {
  ...
  for (c0 = 0; c0 <= floord(NI - 1, 16); c0 += 1)
    for (c1 = 0; c1 <= floord(NJ - 1, 16); c1 += 1)
      if (c0 >= 0 && c1 >= 0)
        for (c2 = 0; c2 <= floord(NK - 1, 16); c2 += 1)
          for (c3 = 16 * c0; c3 <= min(NI - 1, 16 * c0 + 15); c3 += 1)
            for (c4 = 16 * c1; c4 <= min(NJ - 1, 16 * c1 + 15); c4 += 1) {
              if (c2 == 0)
                E[c3][c4]=0;
              for (c5 = 16 * c2; c5 <= min(NK - 1, 16 * c2 + 15); c5 += 1)
        ...
        for (c2 = 0; c2 <= floord(NM - 1, 16); c2 += 1)
          for (c3 = 16 * c0; c3 <= min(NJ - 1, 16 * c0 + 15); c3 += 1)
            for (c4 = 16 * c1; c4 <= min(NL - 1, 16 * c1 + 15); c4 += 1) {
              if (c2 == 0)
                F[c3][c4]=0;
              for (c5 = 16 * c2; c5 <= min(NM - 1, 16 * c2 + 15); c5 += 1)
