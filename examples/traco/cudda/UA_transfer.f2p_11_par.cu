...
int DIM1;
int DIM2;
__global__ void seq_kernel0(int * N1, int * N2, int * N3, float qbnew[DIM1][DIM2][2], float tmor[DIM1][DIM2], float tmp[DIM1][DIM2])
{
int col;
int i;
int j;
...
gpuBytes=(((DIM1*DIM2)*2)*sizeof (float));
CUDA_SAFE_CALL(cudaMalloc(((void * *)( & gpu__qbnew)), gpuBytes));
CUDA_SAFE_CALL(cudaMemcpy(gpu__qbnew, qbnew, gpuBytes, cudaMemcpyHostToDevice));
gpuBytes=((DIM1*DIM2)*sizeof (float));
CUDA_SAFE_CALL(cudaMalloc(((void * *)( & gpu__tmor)), gpuBytes));
CUDA_SAFE_CALL(cudaMemcpy(gpu__tmor, tmor, gpuBytes, cudaMemcpyHostToDevice));
gpuBytes=((DIM1*DIM2)*sizeof (float));
CUDA_SAFE_CALL(cudaMalloc(((void * *)( & gpu__tmp)), gpuBytes));
CUDA_SAFE_CALL(cudaMemcpy(gpu__tmp, tmp, gpuBytes, cudaMemcpyHostToDevice));
#pragma omp parallel for shared(N1, N2, N3, qbnew, tmor, tmp) private(col, i, j)
#pragma cuda ainfo kernelid(0) procname(seq) 
#pragma cuda gpurun nog2cmemtr(N1, N2, N3, qbnew, tmor) 
seq_kernel0<<<dimGrid0, dimBlock0, 0, 0>>>(gpu__N1, gpu__N2, gpu__N3, ((float (*)[DIM2][2])gpu__qbnew), ((float (*)[DIM2])gpu__tmor), ((float (*)[DIM2])gpu__tmp));
gpuBytes=((DIM1*DIM2)*sizeof (float));
CUDA_SAFE_CALL(cudaMemcpy(tmp, gpu__tmp, gpuBytes, cudaMemcpyDeviceToHost));
CUDA_SAFE_CALL(cudaFree(gpu__tmp));
gpuBytes=((DIM1*DIM2)*sizeof (float));
CUDA_SAFE_CALL(cudaFree(gpu__tmor));
gpuBytes=(((DIM1*DIM2)*2)*sizeof (float));
CUDA_SAFE_CALL(cudaFree(gpu__qbnew));
gpuBytes=sizeof (int);
