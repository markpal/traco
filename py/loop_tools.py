...
def CorrectLoop(_file):
    ...
    z1 = re.compile(r"\W*if\W*\(")
    z2 = re.compile(r"\)\W*{")
    z3 = re.compile(r"\W*for\W*\(")
    z4 = re.compile(r" ")
...
def ReadStatementNests(plik, dane):
    ...
    for line in petit_loop:
        if("for " in line and not("endfor" in line)):
            idx_var = re.findall(r"for.*=", line)[0].replace("for", "").replace("=", "").replace(" ", "")
            lb = re.findall(r"=.*to", line)[0].replace("to", "").replace("=", "").replace(" ", "")
            ub = re.findall(r"to.*do", line)[0].replace("to", "").replace("do", "").replace(" ", "")
