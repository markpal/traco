...
Relation IterateClosure(NOT_CONST Relation &r, int n) {

Relation s = consume_and_regurgitate(r);
Relation R = Identity(s.n_inp());

Relation R_new,R_old,tc,h,h1,h2;


for (DNF_Iterator c(s.query_DNF()); c; c++) {
    R = Union(R, TransitiveClosure(Relation(s, c.curr()), 1,Relation::Null()));
  }
  ...
  do{
      R_old = copy(R_new);
      R_new = Relation::Null();
      R_new = Composition(copy(R), copy(R_old));
      R_new.simplify();
      R_new = Difference(R_new, Identity(s.n_inp()));
      h = Hull(copy(R_new),false,1,Null_Relation());
      h1 = Difference(copy(h), copy(R_new));
      h2 = Difference(h, h1);

      if (h2.number_of_conjuncts() < R_new.number_of_conjuncts()) {
	R_new = Union(h2, Identity(s.n_inp()));
      }

      R_new = Union(R_new, Identity(s.n_inp()));
    }while((!Must_Be_Subset(copy(R_new), R_old)) && (--n > 0));

  if (n == 0) {
    R_new = Relation::Null();
    tc = TransitiveClosure(DeltasToRelation(Deltas(copy(s)),s.n_inp(),s.n_out()), 1, Relation::Null());
    tc = Restrict_Domain(tc,Domain(copy(s)));
    tc = Restrict_Range(tc,Range(s));
    tc = Intersection(tc, Relation::Unknown(tc));
  }
  else {
    tc = Difference(R_new, Identity(s.n_inp()));
  }
