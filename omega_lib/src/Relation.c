...
/* Tomasz Klimek */
int Relation::get_max_wildcards_number() {
  int nWildcards = 0;
  int nMaxWildcards = 0;
  for (DNF_Iterator c(this->query_DNF()); c; c++) {
    nWildcards = (*c)->n_Wilcards();
    if (nWildcards > nMaxWildcards) {
      nMaxWildcards = nWildcards;
    }
  }

  return nMaxWildcards;
