	...
	case op_entry:
	    ...
	    for( dd = n->nodeddout; dd != NULL; dd = dd->ddnextsucc )
		{
			...
			symtabentry *var = get_nodevalue_sym(get_nodevalue_node((dd->ddpred==Entry)?dd->ddsucc:dd->ddpred));
				...
				if(! write_as_graph) {
				  fprintf(printout, "%s\n", brdd_print( dd ));
/*******ZAPAMIETANIE TYPU RELACJI Z PETITA************/
					petitRelationsType.append(get_ddtype_name(dd->ddtype));
/*****************************************************/
...
/***********ZPAMIETANIE RELACJI Z PETITA**************/
						//pRels.append(copy(r));
						petitRelations.append(copy(r));
						petitFrom.append(dd->ddpred->nodesequence);
						petitTo.append(dd->ddsucc->nodesequence);
						std::string linia(brdd_print(dd));
						petitLine.append(linia);
                        petitVar.append(var->symname);
/*****************************************************/
...
/**********ZAPAMIETANIE RODZAJU WEKTORA ZALEZNOSCI - WEKTOR ZMIENNY ************/
						//dodajemy wartosc FALSE
						petitRelationsVecType.append(false);
						petitDepVector.append(d);
...
/**********ZAPAMIETANIE RODZAJU WEKTORA ZALEZNOSCI - WEKTOR STALY ************/
						 //dodajemy wartosc TRUE
						 petitRelationsVecType.append(true);
						 petitDepVector.append(d);
/*****************************************************************************/
					    }
				      }
	...
	//print_rels(petitRelations, petitRelationsVecType, petitDepVector, petitFrom, petitTo, petitRelationsType);
} /* write_deps */
...
int brdd_graph(char *filename, char *inputfile, bool graph_by_statement, bool popup_vcg, Tuple<Relation> *Rels)
{
...
void writeRelations(Tuple<Relation> *rels)
{
    pRels[1].print();
    rels->append(pRels[1]);
