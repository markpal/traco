...
void write_depends(int found, char *fname, Tuple<Relation> *_petitRelations)
    {
    if (found)
        {
        if(petit_args.graph_write_dd_as_graph)
        {
            brdd_graph(petit_args.DepWrFile,fname,petit_args.graph_by_statement,true, _petitRelations);
