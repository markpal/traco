  ...
  for (int o=1; o<=info.length(); o++) {
    if (o != stm) {
      ...
      if (Intersection(copy(info[o].IS),copy(IS_mask)).is_satisfiable()) {
        std::cerr << "Can't extend level " << level <<
          " of statement " << stm <<
          " because of interference from I.S. of statement " << o <<std::endl;
        std::cerr << "Statement " << o << "'s I.S. is " << info[o].IS;
        return false;
      }
    }
    else {
...
void DoDebug2(const char *message, const Tuple<stm_info> &info) {
...
void DoDebug(const char *message, int effort, const Tuple<stm_info> &info, const Relation &known) {
  ...
  fprintf(DebugFile, "%s", result.c_str());
}
