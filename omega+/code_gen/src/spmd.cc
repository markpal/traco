...
std::string program_head(std::string Decls, int timeDepth) {
  ...
  assert (timeDepth > 0);
  std::string index_vars = "    int t1";
  for(int ind = 2; ind <= timeDepth; ind++)
    index_vars += ", t" + to_string(ind);
  index_vars += ";\n";

  return s + index_vars;
}

static Relation select_output(Relation R, int level) {
...
#if 0
static void move_input_and_output(int from_input, int from_output, int to, Relation &R) {
...
void move_just_output(int from, int to, Relation &R) {
// Output tuple:
// [i_1,..i_to,..,i_from,..,i_n] becomes [i1,..,ifrom,..,ito,..,i_n]
//                                                       ^^^ Ito, get it?
//                                                       You know, as in Judge?
...
void move_just_input(int from, int to, Relation &R) {
// Input tuple:
// [i_1,..i_to,..,i_from,..,i_n] becomes [i1,..,ifrom,..,ito,..,i_n]
//                                                       ^^^ Ito, get it?
//                                                       You know, as in Judge?
...
static void extend_and_equate(Relation &R) {
  R = Extend_Domain(R);
  R = Extend_Range(R);
...
static void extend_mappings(Relation &time,     /* Relation&space, */
...
static Tuple<int> move_cyclic_loop_outward(SetTuple &Space, RelTuple &Time,
...
std::string SPMD_GenerateCode(std::string Decls, SetTuple &Space, RelTuple &Time,
...
#define maxStmts 20
      ...
      for (GEQ_Iterator g(low[i].single_conjunct()); g; g++) {
        if ((*g).get_coef(input_var(1)) > 0) {
          ...
          if ((*g).is_const(input_var(1))) {
            ...
            start += GetString(outputLBasRepr(&ocg, *g, low[i], input_var(1), 1, EQ_Handle(), Relation()));
          } 
          seenLB = true;
        }
      }
      ...
      for (GEQ_Iterator g(high[i].single_conjunct()); g; g++) {
        if ((*g).get_coef(input_var(1)) < 0) {
          ...
          if ((*g).is_const(input_var(1))) {
            ...
            finish += GetString(outputUBasRepr(&ocg, *g, high[i],input_var(1), 1, EQ_Handle()));
          } 
          seenUB = true;
        }
      }
  ...
  std::string loopNest = MMGenerateCode(Time, Local, NameInfo, Known, 
                                        overheadEffort);
...
std::string SPMD_GenerateCode(std::string Decls, SetTuple &Space, RelTuple &Time,
  ...
  for(int j = 1; j <= processorDim; j++) {
    // declare ubj, lbj
    lowerBounds.append(new Free_Var_Decl("lb" + to_string(j)));
    upperBounds.append(new Free_Var_Decl("ub" + to_string(j)));
    my_procs.append(new Free_Var_Decl("my_proc" + to_string(j)));
  }
  Tuple<spmd_stmt_info *> NameInfo;
