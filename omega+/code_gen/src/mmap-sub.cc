...
static std::string desperate_check_mod_kludge_for_time_skewing(Relation &R, int o) {
...
#endif
    return "intMod(" + std::string(In_8->char_name()) + "-8*" +
      std::string(In_4->char_name()) + "+" + to_string(const_term) + ",8)";
...
#endif
    return "intDiv(intMod(8*intDiv(" + std::string((*In_8).char_name()) + 
      "-8*" + std::string((*In_4).char_name()) + "+" + to_string(const_term) + ",8),16),8)";
...
std::string get_sub(Relation &R, int o, Substitutions &subs) {
  ...
  if (subs.substituted(ov)) {
    ...
    return "intMod(" + to_string(m.input_var_coef) + "*" + std::string(R.input_var(m.input_var)->char_name()) + "+" + to_string(m.offset) + "," + to_string(m.modbase) + ")";
  }
  else {
    std::string result = "";
    if (R.n_out() >= 3)
    ...
    if (result != std::string("")) {
      ...
      if (DONT_QUIT_FOR_FAILED_SUBSTITUTION) {
        if (tcodegen_debug) {
          fprintf(DebugFile, "%s%s\n", debug_mark_cp, "failed to find substitution.\n");
          if (tcodegen_debug > 1) {
            fprintf(DebugFile, "%s%s\n", debug_mark_cp, "In prefix_print form, R is:");
        ...
        std::cerr << "The current tcodegen implementation needs to find a subsitution for each\n";
        std::cerr << "output variable, but can't do so for output variable #" << o << " in\n";
