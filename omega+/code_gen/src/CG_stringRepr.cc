...
default_stmt_info::default_stmt_info(int stmt): stmt_num(stmt) {
}

std::string default_stmt_info::debug_name() const {
  return std::string("s")+to_string(stmt_num);
}

CG_outputRepr* default_stmt_info::place_holder() const {
  return new CG_stringRepr(std::string("s")+to_string(stmt_num));
}
