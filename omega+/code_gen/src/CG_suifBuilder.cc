...
CG_suifBuilder::CG_suifBuilder(proc_symtab *symtab): symtab_(symtab) {
}

CG_suifBuilder::~CG_suifBuilder() {
}
...
//-----------------------------------------------------------------------------
CG_outputRepr* CG_suifBuilder::CreateStmtList(CG_outputRepr *singleton) const {
  if (singleton == NULL) {
    return new CG_suifRepr(new tree_node_list);
  }

  tree_node_list *tnl = static_cast<CG_suifRepr *>(singleton)->tnl_;
  
  if (tnl == NULL) {
    tnl = new tree_node_list;
    tnl->append(new tree_instr(static_cast<CG_suifRepr *>(singleton)->op_.instr()));
  }
...
CG_outputRepr* CG_suifBuilder::StmtListInsertLast(CG_outputRepr *list, 
