...
static std::string parenthesize_vars_hack(const std::string &expr) {
  std::string answer = "";
...
*/
static std::string output_vars_as_subscripts(Relation &R) {
  int dnf_len = R.query_DNF()->length();
  if (dnf_len>1) {
    std::cerr << "The current tcodegen implementation requires a single-conjunct Relation,\n";
    std::cerr << "to express output variables in terms of inputs and symbolic consts.\n";
    std::cerr << "If possible, replace this Relation with several single-conjunct Relations:\n";
    std::cerr << R.print_with_subs_to_string();
    exit(1);
  }
...
// EX:         a[i-1,t-1]
/*****************************************************************************/
static std::string arrayref(PartialMMap memory, Relation DataFlow) {
  std::string answer;
...
/*****************************************************************************/
static std::string printDefine(int &statementCount, int input) {
  int i;
  std::string s= "";
  s += "#define s" + to_string(statementCount) + "(";    // #define sx(
  statementCount++;
  i=1;
  while (i<=input) {
    s += "t" + to_string(i);      // #define sx(tx
    if (i!=input)
      s += ",";         // #define sx(tx,
    i++;
  }
...
/*****************************************************************************/
static std::string printDefStmts(Tuple<stm_info> &In) {
    ...
    if (!(NewInfo[i].IS.is_satisfiable())) {
      {
        for (int j=1; j<=NewInfo[i].map.partials.size(); j++)
          if (NewInfo[i].map.partials[j].mapping.is_satisfiable())
            std::cerr<<"A partial write mapping (info["<<i<<"].map.paartials["<<j<<"].mapping) is satisfiable while the iteration-space is not"<<std::endl;
      }
      {
        for (int j=1; j<=NewInfo[i].read.size(); j++)
          for (int k=1; k<=NewInfo[i].read[j].partials.size(); k++)
            if (NewInfo[i].read[j].partials[k].dataFlow.is_satisfiable())
              std::cerr<<"A partial read dataFlow (info["<<i<<"].read["<<j<<"].partials["<<k<<"].dataFlow) is satisfiable while the iteration-space is not"<<std::endl;
      }
      {
      ...
      if (NewInfo[i].map.partials.size()==0 && !empty)
        std::cerr<<"No satisfiable partial writes left in info["<<i<<"].map.partials"<<std::endl;
      count=0;
      empty=true;
      for (int j=1; j<=NewInfo[i].read.size(); j++) {
        empty=true;
      ...
      if (count==0 && !empty)
        std::cerr<<"No satisfiable partial reads left in info["<<i<<"].read"<<std::endl;
    }
  }
...
/*****************************************************************************/
std::string tcodegen(int effort, Tuple<stm_info> &info, const Relation &known1, bool check_input) {
  std::string result="";
  RelTuple R;
  SetTuple S;
  Relation identity;
  ...
  if (check_input) {
    ...
    if (known.n_set() != 0 && known.n_set() != arity) {
      std::cerr << "Inconsistent arity for \"known\": " << known.n_set() <<std::endl;
      value = false;
    }
    if (!value)
      return "";
  ...
  if (tcodegen_debug) {
    ...
    for (int a=1; a<=R.size(); a++) {
      debug_info += debug_mark +
        "Argument " + to_string(a) + " relation and set are\n";
    ...
    fprintf(DebugFile, "%s", debug_info.c_str());
    result += debug_info;
  }
