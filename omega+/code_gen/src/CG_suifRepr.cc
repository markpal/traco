...
tree_node_list* CG_suifRepr::GetCode() {
  return tnl_;
}

operand CG_suifRepr::GetExpression() {
  return op_;
} 
...
suif_stmt_info::suif_stmt_info(CG_suifRepr *repr): repr_(repr) {
}

std::string suif_stmt_info::debug_name() const {
  return std::string("");
}

CG_outputRepr* suif_stmt_info::place_holder() const {
  return repr_->clone();
}
