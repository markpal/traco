...
bool checkIS(Tuple<stm_info> info)
{
  ...
  for (int count=1; count<=info.size(); count++) {
	...
	for (int pNext=pCount+1; pNext<=info[count].read[readCount].partials.size(); pNext++) {
      ...
      std::cerr << "Statement "<<count<<" Read "<<readCount<< "'s partials "<<pCount<<" and "<<pNext<<" intersect"<<std::endl;
	    value=false;
	  }
	}
      }
      ...
      if ( ((Difference(readTest,IS)).is_satisfiable()) || 
	   ((Difference(IS2,readTest2)).is_satisfiable()) ) {
        std::cerr << "Statement "<<count<<" Read "<<readCount<<"'s relations does not describe IS: ";
	...
	if (copy(info[count].IS).is_satisfiable())
	  {
      std::cerr << "statement "<<count<<"'s has no writes, but IS: ";
	    info[count].IS.print_with_subs(stderr);
	    value = false;
	  }
	continue;
    ...
    std::cerr << "Statement "<<count<<"'s write partials "<<wCount<<" and "<<wNext<<" intersect"<<std::endl;
	  value=false;
	}
      }
    }
    ...
    if ( ((Difference(writeTest,IS)).is_satisfiable()) || 
	 ((Difference(IS2,writeTest2)).is_satisfiable()) ) {
      std::cerr << "statement "<<count<<"'s write relation does not describe IS: ";
      info[count].IS.print_with_subs(stderr); 
      value=false;
    }
  } // statment loop 
...
bool checkDataFlow(Tuple<stm_info> &info)
{
  ...
  for(int i=1; i<=info.size(); i++)
	...
	if (Difference(copy(info[i].read[j].partials[k].dataFlow),
	  ...
	  {
      std::cerr<<"DataFlow goes back in time in statement "<<i<<", in read "<<j<<", in partial read "<<k<<std::endl;
      std::cerr<<"DataFlow relation: "<<info[i].read[j].partials[k].dataFlow;
	    value=false;
	  }
  return value;
}
...
bool checkMemoryMapping(Tuple<stm_info> info)
{
   ...
   if (debugthis)
	...
	{ 
    std::cerr<<"i= "<<i<<"  ArrayMapping[i].var= "<<ArrayMapping[i].var<<"  ArrayMapping[i].mapping= "<<ArrayMapping[i].mapping;
	}
     }
   ...
   for(int i=1; i<=info.size(); i++)
	 ...
	 for(int k=1; k<=info[i].read[j].partials.size(); k++)
       ...
       std::cerr<<"for dataFlow: "<<dF;
       std::cerr<<"  in statement "<<i<<", in read "<<j<<", in partial read "<<k<<std::endl;
       std::cerr<<"  the memory map is invalid for the Array \""<<ArrayMapping[m].var<<"\"."<<std::endl;
		   Relation shut_up_about_non_constant_refs = copy(sample_bad);
       std::cerr<<"  intersection(dataflow, AfterSameMem.mapping) includes " << shut_up_about_non_constant_refs << std::endl;
		   ...
		   for(int i=1; i<=ArrayMapping.size(); i++)
			 ...
			 {
         std::cout << "  that address is written, via memory map " << i << ", in iterations " << writes << std::endl;
			 }
		     }
	 ...
	 if (!bounds.is_obvious_tautology())
	   {
	    ...
	    if (mapping.n_out() != bounds.n_set())
	      {
          std::cerr <<"Array mapping has wrong number of subscripts for:"<<std::endl;
          std::cerr <<"bounds: "<<bounds<<"mapping: "<<mapping;
	       value=false;
	      }
	    else if (!Must_Be_Subset(Range(copy(mapping)),copy(bounds)))
	      {
          std::cerr<<"Array mapping not within memory map bounds for:"<<std::endl<<"bounds: "<<bounds<<"mapping: "<<mapping;
	       value=false;
	      }
	   }
        }
      ...
      for(j=1; j<=info[i].read.size(); j++)
         for(k=1; k<=info[i].read[j].partials.size(); k++)
	   {
	    if(!Must_Be_Subset(Domain(copy(info[i].read[j].partials[k].dataFlow)),copy(info[info[i].read[j].partials[k].from].IS)))
	      {
          std::cerr<<"Data flow source not within iteration space for " <<
		  "statement " << i << ", read " << j << ", partial " << k <<
		  " (source stmt " << info[i].read[j].partials[k].from << ")\n";
          std::cerr << "dataFlow:  " << info[i].read[j].partials[k].dataFlow;
          std::cerr << "source IS: " << info[info[i].read[j].partials[k].from].IS;
		value=false;
	      }
	   }
   return value;
...
bool check_arities(Tuple<stm_info> &info)
{
  ...
  for(int i=1; i<=info.size(); i++)
	...
	{
    std::cerr << "Inconsistent I.S. dimensions for statements 1 (" <<
	    info[1].IS.n_set() <<
	    ") and " << i << " (" << info[i].IS.n_set() << ")" << std::endl;
	  value = false;
	}
      ...
      for(int m=1; m<=info[i].map.partials.size(); m++)
	{
	  if (info[i].map.partials[m].mapping.n_inp() != arity)
	    {
        std::cerr << "Inconsistent I.S. dimensions (" <<
		info[i].map.partials[m].mapping.n_inp() << 
		") used in memory mapping " <<
          m << " of statement " << i << std::endl;
	      value = false;
	    }
	}
	...
	for(int k=1; k<=info[i].read[j].partials.size(); k++)
	  {
	    if (info[i].read[j].partials[k].dataFlow.n_inp() != arity)
		  ...
		  ") used in source of dataflow " << k <<
            " of read " << j << " of statement " << i << std::endl;
		value=false;
	      }
