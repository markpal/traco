...
/* Tomasz Klimek for Rk */
#include <../../omega_calc/include/omega_calc/AST.h>
...
/* *********************************************** */
/*     Find minimal dependence distance vector     */
/* *********************************************** */

Relation findMinimal(NOT_CONST Relation &r){
...
int min,other;
 ...
 for (DNF_Iterator c1(s.query_DNF()); c1; c1++, i++) {
  if (i==0){
   cmin = c1.curr();
  }
  else{
   for(EQ_Iterator e1i=(*c1)->EQs(), e2i=cmin->EQs(); e1i && e2i; e1i++, e2i++){
     min = (*e2i).get_const();
     other = (*e1i).get_const();

     if ((min > 0 && other < 0) || (min < 0 && other > 0)) {
      (*e2i).update_const(-min);
      continue;
     }

     if (min < 0 && other < 0){
      if (other > min) (*e2i).update_const(-min+other);
      continue;
     }
...
Relation MinDependenceDistance(NOT_CONST Relation &r){

Relation s = consume_and_regurgitate(r);
Relation delta = Relation::False(s.n_inp());
Relation deltaTmp,cp,min,max,MIN,MAX;

for (DNF_Iterator c(s.query_DNF()); c; c++) {
  deltaTmp = Deltas(Relation(s, c.curr()));
  
  cp = Cross_Product(copy(deltaTmp),copy(deltaTmp));
  
  min = Difference(copy(deltaTmp),Range(Intersection(cp,LexForward(cp.n_inp()))));
  min = Project_Sym(min);

  cp = Cross_Product(copy(min),copy(min));
  max = Difference(copy(min),Domain(Intersection(copy(cp),LexForward(cp.n_inp()))));
  min = Difference(min,Range(Intersection(cp,LexForward(cp.n_inp()))));
  MIN = Union(min, max);

  cp = Cross_Product(copy(deltaTmp),copy(deltaTmp));
  max = Difference(deltaTmp,Domain(Intersection(cp,LexForward(cp.n_inp()))));
  max = Project_Sym(max);

  cp = Cross_Product(copy(max),copy(max));
  min = Difference(copy(max),Domain(Intersection(copy(cp),LexForward(cp.n_inp()))));
  max = Difference(max,Range(Intersection(cp,LexForward(cp.n_inp()))));
  MAX = Union(min, max);
  
  delta = Union(delta, findMinimal(Union(MIN,MAX)));
 }
...
/* ************************************* */
/*     Approximate input relation to     */
/*          uniform relation             */
/* ************************************* */

Relation ApproxToUniform(NOT_CONST Relation &r){
 ...
 for (DNF_Iterator c(s.query_DNF()); c; c++) {
   appR = DeltasToRelation( MinDependenceDistance(Relation(s, c.curr())), s.n_inp(),s.n_out());
   rTmp = Restrict_Domain(copy(appR), Domain(Relation(s, c.curr())));
   rTmp = Restrict_Range(rTmp, Range(Relation(s, c.curr())));
   
   if (Must_Be_Subset(Domain(Relation(s, c.curr())),Domain(copy(rTmp))) && \
       Must_Be_Subset(Range(Relation(s, c.curr())),Range(copy(rTmp)))) {
         R = Union(R, rTmp);
         continue;
   }
   
   appR=Restrict_Domain(appR, Hull(Union(Domain(Relation(s,c.curr())),Range(Relation(s,c.curr()))),false,1,Null_Relation()));
   appR=Restrict_Range(appR, Hull(Union(Domain(Relation(s,c.curr())),Range(Relation(s,c.curr()))),false,1,Null_Relation()));

   if (Must_Be_Subset(Domain(Relation(s, c.curr())),Domain(copy(appR))) && \
       Must_Be_Subset(Range(Relation(s, c.curr())),Range(copy(appR)))){
         R = Union(R, Intersection(appR, Relation::Unknown(appR)));
         continue;
   }
...
/*     or multiple conjunct uniform relation     */
/* ********************************************* */
...
return Project(s, s.output_var(s.n_out()));
*/

/* 
 uniformR = ApproxToUniform(copy(s));
 deltaSet = Project_Sym(Deltas(copy(uniformR)));
 int nConjuncts = deltaSet.number_of_conjuncts();
 kTab = (Variable_ID*)calloc(nConjuncts, sizeof(Variable_ID));
 eqTab = (EQ_Handle*)calloc(R.n_inp(), sizeof(EQ_Handle));
 F_And* R_root = R.add_and();
 F_Exists* exists = R_root->add_exists();
 F_And* existsStuff = exists->add_and();
 
 for(i=0; i<nConjuncts; i++){
      asprintf(&tmp, "k%d",i+1);
      kTab[i] = exists->declare(tmp);
      (existsStuff->add_GEQ()).update_coef(kTab[i],1);
      free(tmp);
 }

 for(i=0; i<R.n_inp(); i++) {
      eqTab[i] = existsStuff->add_EQ();
      eqTab[i].update_coef(R.output_var(i+1),-1);
      eqTab[i].update_coef(R.input_var(i+1),1);
 }
 
 i=0;
 for (DNF_Iterator c(deltaSet.query_DNF()); c; c++, i++, j=0) {
    for(EQ_Iterator ei=(*c)->EQs(); ei; ei++, j++) {
         eqTab[j].update_coef(kTab[i], (*ei).get_const());
    }
 }


 EQ_Handle eqK = existsStuff->add_EQ();
 asprintf(&tmp, "k");
 k = R.get_local((globalDecls->extend(tmp))->g);
 eqK.update_coef(k,-1);

 for(i=0; i<nConjuncts; i++) {
      eqK.update_coef(kTab[i],1);
 }

 GEQ_Handle geqK = existsStuff->add_GEQ();
 geqK.update_coef(k,1);
 geqK.update_const(-1);

 
 R = Restrict_Domain(R, Domain(copy(uniformR)));
 R = Restrict_Range(R, Range(uniformR));
...
/*
 ...
 for (DNF_Iterator c(s.query_DNF()); c; c++) {
  TMP = Relation(s, c.curr());
  eqK = TMP.and_with_EQ();
  eqK.update_coef(TMP.get_local(k),1);
  eqK.update_const(-1);

  TMP.finalize();
  TMP.simplify();
  R_out = Union(R_out, TMP);
 }
*/

// F_And* R_root = R.add_and();
// EQ_Handle eqK = R.and_with_EQ();
// eqK.update_coef(k,1);
// eqK.update_const(-1);
...
/*
   R_out.print();
   R_out = Project(R_out, R_out.output_var(R_out.n_out()));
*/ 
 
// R_out = Project(R_out, R_out.input_var(R_out.n_inp()));

/*
return Intersection(R_out, Relation::Unknown(R_out)); 
...
/* ********************************************* */
/*        Check if all relations are             */
/*        described in a common region           */
/* ********************************************* */


bool AllInRegion(NOT_CONST Relation &r, NOT_CONST Relation &region){
...
Relation R;
 ...
 R = DeltasToRelation(Deltas(copy(s)),s.n_inp(),s.n_out());
 ...
 if (Must_Be_Subset( copy(s), copy(R)) && Must_Be_Subset( copy(R), copy(s)))
  return true;
...
bool checkAllInRegion(NOT_CONST Relation &r){

Relation s = consume_and_regurgitate(r);

return AllInRegion(copy(s), Union(Domain(copy(s)), Range(copy(s))));

}

 /* ********************************* */
 /*       Get a conjunction for       */
 /*   a given number from relation    */
 /* ********************************* */
