...
/* Tomasz Klimek */
static Relation composeN_Naive(NOT_CONST Relation &r, int &condition) {
...
/*************************************** */
/*  Calculate R* in in naive procedure   */
/*					 */
/*  Tomasz Klimek 05-06-2010		 */
/*************************************** */

Relation SemiNaiveClosure(NOT_CONST Relation &r) {
...
int condition=1;
 ...
 for (DNF_Iterator c(s.query_DNF()); c; c++) {
     ...
     //  break;
     //}
     R = Union(R, Relation(s, c.curr()));
 }

 if (condition > 0) {
   tc = composeN_Naive(R, condition);
 }
...
Relation IterateClosure(NOT_CONST Relation &r) {
...
/****************************************** */
/*  Calculate relation R - (R compose R+)   */
/*					    */
/*  Tomasz Klimek 05-06-2010		    */
/****************************************** */

Relation TransitiveReduction(NOT_CONST Relation &r){

Relation s = consume_and_regurgitate(r);

Relation tc = IterateClosure(copy(s));
Relation tc2 = Composition(copy(s), tc);

return Difference(s, tc2);
}
...
/*
  for (DNF_Iterator c(s.query_DNF()); c; c++) {
     if (TransitiveClosure(Relation(s, c.curr()), 1,Relation::Null()).is_inexact()) {
    	 printf("\nAproksymacja tranzytywnego domknięcia\n");
...
/*
  commonRegion = RectHull(Union(Domain(copy(s)),Range(copy(s))));
   
  if (AllInRegion(copy(s), copy(commonRegion))) {
	  printf("\nOpisane na pełnym regionie\n");
	  return TransitiveClosure3(s, commonRegion);
  }
*/  

  commonRegion = Relation::Null();
  commonRegion = Union(Domain(copy(s)),Range(copy(s)));
...
*/
  ...
  for (; N > 1;){
	...
	if ( j+1 == N) {
		...
		regionTmp = RectHull(copy(commonRegion));
		
		R = relationsOnCommonRegion( copy(inputRelations), copy(regionTmp) );

		if (AllInRegion( copy(R), copy(regionTmp) )) {
			printf("\nOpisane na prostokącie\n");
			tc = Union( tc, TransitiveClosure3(R, regionTmp));
		}
			...
			regionTmp = Hull(copy(commonRegion),false,1,Null_Relation());
			...
			if (AllInRegion( copy(R), copy(regionTmp) )) {
				...
				tc = Union( tc, TransitiveClosure3(R, regionTmp));
				  ...
				  /* *************************** */
				  /*    Check on trurly region   */
				  /* *************************** */
				
				  R = Relation::Null();
				  R = relationsOnCommonRegion( copy(inputRelations), copy(commonRegion) );
				
				  if (AllInRegion( copy(R), copy(commonRegion) )) {
					  printf("\nOpisane na domain i range\n");
					  tc = Union( tc, TransitiveClosure3(R, commonRegion));
				  }
			}
		
/* Tą klamre zakomentować jesli nie na prostokącie */
  ...
  for (DNF_Iterator c(s.query_DNF()); c; c++) {
	if (!Must_Be_Subset(Relation(s, c.curr()), copy(tc))) {
		printf("\nNie jest podzbiorem\n");
		tc = Union( tc, TransitiveClosure(Relation(s, c.curr()), 1,Relation::Null()));
	}
  }

 tc = SemiNaiveClosure(tc);
