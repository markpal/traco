...
/* Tomasz Klimek */
%token ITERATE_CLOSURE APPROX_CLOSURE TRANSITIVE_REDUCTION APPROX_TO_UNIFORM RK TC SEMI_NAIVE_CLOSURE ADVANCED_ITERATE_CLOSURE ALL_IN_REGION MIN_DEPENDENCE_DIST
%token TEST
...
%destructor {delete $$;} relation builtRelation tupleDeclaration formula optionalFormula context reachable_of constraintChain varDecl varDeclOptBrackets relPairList relTripList reachable statementInfoList statementInfoResult statementInfo reads oneread partials partial partialwrites partialwrite
...
inputItem : ';' /*empty*/
          ...
          | ALL_IN_REGION relation {
          	flushScanBuffer();
            try {
              int c = checkAllInRegion(*$2);
              printf("\n%s\n", c ? "True" : "False");
            }
            catch (const std::exception &e) {
              std::cout << e.what() << std::endl;
 ...
 */
          | SPMD blockAndProcsAndEffort relTripList';' {
            ...
            try {
              ...
              std::string s = SPMD_GenerateCode(std::string(""), $3->space, $3->time, $3->ispaces, 
                                           names,
...
statementInfoResult : statementInfoList {$$ = $1;}
...
reads : reads ',' oneread {$$ = $1; $$->read.append(*($3)); delete $3;}
      | oneread {$$ = new stm_info; $$->read.append(*($1)); delete $1;}
;

oneread :  '[' partials ']' { $$ = $2; }
;

partials : partials ',' partial {$$ = $1; $$->partials.append(*($3)); delete $3;}
         | partial {$$ = new Read; $$->partials.append(*($1)); delete $1;}
;

partial : INT ',' relation {$$ = new PartialRead; $$->from = $1; $$->dataFlow = *($3); delete $3;}
;
...
relation : OPEN_BRACE {need_coef = true; relationDecl = new Declaration_Site();}
         ...
         | ITERATE_CLOSURE relation  %prec p8 {
           $$ = new Relation();
           try {
             *$$ = IterateClosure(*$2);
           }
         ...
         | ADVANCED_ITERATE_CLOSURE relation  %prec p8 {       
           $$ = new Relation();
           try {
             *$$ = AdvancedIterateClosure(*$2);
           }
           catch (const std::exception &e) {
             delete $$;
             delete $2;
         ...
         | SEMI_NAIVE_CLOSURE relation  %prec p8 {
	       $$ = new Relation();
           try {
             *$$ = SemiNaiveClosure(*$2); 
           }
         ...
         | APPROX_CLOSURE relation   %prec p8 {
	       $$ = new Relation();
           try {
             *$$ = ApproxClosure(*$2); 
           }
         ...
         | MIN_DEPENDENCE_DIST relation  %prec p8 {
	       $$ = new Relation();
           try {
             *$$ = MinDependenceDistance(*$2); 
           }
           catch (const std::exception &e) {
             delete $$;
             delete $2;
         ...
         | APPROX_TO_UNIFORM relation   %prec p8 {
	       $$ = new Relation();
           try {
             *$$ = ApproxToUniform(*$2);
           }
	     ...
	     | TRANSITIVE_REDUCTION relation  %prec p8 {
	       $$ = new Relation();
           try {
             *$$ = TransitiveReduction(*$2);
           }
           catch (const std::exception &e) {
             delete $$;
             delete $2;
