...
void printExp(Exp *x) {
  printf(coef_fmt,x->constantTerm);
  foreach_map(r,Variable_Ref*,c,coef_t,x->coefs,
              printf("+"coef_fmt"%s",c,(const char *)r->name));
}

Exp::Exp(coef_t c) : coefs(0) {
  constantTerm = c;
}

AST_constraints::AST_constraints(ExpList *f, Rel_Op r, AST_constraints *o) {
  others = o;
  rel_op = r;
  first = f;
}

AST_constraints::AST_constraints(ExpList *f, Rel_Op r, ExpList *s) {
  others = new AST_constraints(s);
  rel_op = r;
  first = f;
}

AST_constraints::AST_constraints(ExpList *f){
  others = 0;
  first = f;
}
...
Exp *negate (Exp *x) {
  x->constantTerm *= -1;
  foreach_map(r,Variable_Ref*,c,coef_t,x->coefs,x->coefs[r]*=-1);
  return x;
}

Exp *add (Exp *x, Exp *y) {
  x->constantTerm += y->constantTerm;
  foreach_map(r,Variable_Ref*,c,coef_t,y->coefs,x->coefs[r]+=c);
  delete y;
  return x;
}
...
Exp *multiply (coef_t c, Exp *x) {
  x->constantTerm *= c;
  foreach_map(r,Variable_Ref*,v,coef_t,x->coefs, v *= c);
  return x;
}
...
Variable_Ref *lookupScalar(char *s) {
  ...
  // printf("Can't find declaration for `%s'\n",s);
...
void install_stride(F_And *F, strideConstraint *s) {
  ...
  foreach_map(r,Variable_Ref*,co,coef_t,s->e->coefs,
...
void install_eq(F_And *F, Exp *e1, Exp *e2) {
  ...
  foreach_map(r,Variable_Ref*,co,coef_t,e1->coefs,
              c.update_coef(r->id(F->relation()),co));
  if (e2) foreach_map(r,Variable_Ref*,co,coef_t,e2->coefs,
                      c.update_coef(r->id(F->relation()),-co));
  c.finalize();
}
...
/*   //foreach(g,Free_Var_Decl*,globals,delete g); */
  ...
  for (Any_Iterator<Variable_Ref *> i= declarations.any_iterator(); i; i++) {
    if ((*i)->g != 0) {
